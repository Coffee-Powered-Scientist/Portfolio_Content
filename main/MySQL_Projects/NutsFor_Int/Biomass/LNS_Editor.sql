...
use NutsF;
DROP TABLE IF EXISTS LN_Biomass, HN_Biomass, HNB_Biomass, LNB_Biomass, Organ_Table, Organ_Table_1, Organ_Table_2,Organ_Table_3, DW_Table, DW_Table_1, DW_Table_2,DW_Table_3;
-- This section aims to automatically load .csv files in
create TABLE LN_Biomass (
	YEAR INT(15),	
    Month INT(15),
    DW_F VARCHAR(50),
	N_CONC_F VARCHAR(50),
...
select*from LN_Biomass;
-- Test out with a manually loaded .csv first
LOAD DATA INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\NutsFor_Data\\Biomass_Data\\Raw_Data\\LNS\\Tree data.csv'
INTO TABLE LN_Biomass
...
-- This inserts the original LN_Biomass Table into the new Organ_Table, splitting up named elements into the different columns
INSERT INTO Organ_Table (YEAR, Month, Site, Biomass_Type, M_Type, Ca, Mg, N, K, S, P)
...
INSERT INTO DW_Table (YEAR, Month, Site, Biomass_Type, DW)
...
UPDATE Organ_Table SET Site = 'LNS';
SET SQL_SAFE_UPDATES = 1;

SET SQL_SAFE_UPDATES = 0;
UPDATE DW_Table SET Site = 'LNS';
SET SQL_SAFE_UPDATES = 1;


Select * FROM Organ_Table;
Select*from DW_Table; -- Check import results, check1 should be an empty column (also, the upload doesn't work without check1